    ...
    def construct(self, briq: VisualBriq) -> VisualBriq:
        ...
        try:
            if briq.mode == InputMode.IMAGE:
                ...
                # TEXT or VIDEO: generate/transform image
            ...
            raw_video_path = self._save_to_qodeyard(briq, video_result.video_path)
            briq.raw_video_path = raw_video_path

            briq.generation_time = time.time() - start_time
            briq.status = BriqStatus.CONSTRUCTED

            logger.info(f"[ConstruQtor] Construction complete: {raw_video_path} "
