...
class VeoBackend:
    ...
    def _generate_veo_video(
        ...
        config_kwargs: Dict[str, Any] = {
            "number_of_videos": min(self.veo_cfg.sample_count, 1),
            "aspect_ratio": aspect,
            "person_generation": self.veo_cfg.person_generation,
            "generate_audio": gen_audio,
        }

        # sample_count > 1: the SDK supports it on Vertex, but this backend
        # only downloads the first result. Warn if configured > 1.
        if self.veo_cfg.sample_count > 1:
            logger.warning(
                f"[Veo] sample_count={self.veo_cfg.sample_count} but only the first "
                f"generated video is downloaded. Set sample_count: 1 to avoid wasted quota."
            )
            config_kwargs["number_of_videos"] = self.veo_cfg.sample_count

        # resize_mode: documented in Vertex REST API for image-to-video but
        # NOT exposed in the google-genai Python SDK's GenerateVideosConfig.
        # Log once if user set a non-default value so they know it's ignored.
        if self.veo_cfg.resize_mode and self.veo_cfg.resize_mode.upper() != "PAD":
            logger.warning(
                ...
                f"Use the REST API directly if resize_mode control is needed."
            )

        # Duration: only set for non-extension modes
        ...
        # Storage URI for Vertex
        if self.veo_cfg.storage_uri and self.provider == VeoProvider.VERTEX:
            config_kwargs["output_gcs_uri"] = self.veo_cfg.storage_uri

        # Compression quality
        cq = self.veo_cfg.compression_quality.upper()
        if cq in ("LOSSLESS", "OPTIMIZED"):
            config_kwargs["compression_quality"] = cq

        # Last frame for first_last_frame mode
        ...
        # Reference images
        if (
            self.veo_cfg.enable_reference_images
        ...
        for attempt in range(1, self.veo_cfg.max_retries + 1):
            try:
                ...
                if any(k in err_str for k in ["auth", "permission", "forbidden", "invalid_api_key"]):
                    ...
                    break
                if "unsupported" in err_str or "invalid" in err_str:
    ...
    def _poll_and_download(
        ...
        for vid_idx, generated in enumerate(response.generated_videos):
            ...
            if video_obj.video_bytes:
                ...
                self._download_gcs(video_obj.uri, output_path)
            else:
    ...
    def _download_gcs(self, uri: str, output_path: Path) -> None:
        """Download a file from GCS URI."""
        ...
        try:
            ...
            raise RuntimeError(
                "[Veo] gsutil not found. Install Google Cloud SDK to download from GCS URIs."
            )
    ...
    def generate_video(self, request, source_image: Path) -> "GenerationResult":
