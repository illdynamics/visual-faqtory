...
class VisualFaQtory:
    ...
    def _build_story_config(self) -> SlidingStoryConfig:
        ...
        return SlidingStoryConfig(
            ...
            video_fps=ps.get("video_fps"), # No default, let TimingResolver handle it
            video_frames=ps.get("video_frames"), # New
            timing_authority=ps.get("timing_authority"), # New
            backend_config=bc,
    ...
    def run(self) -> Path:
        ...
        if audio_cfg.get("enabled", False) and self.base_audio:
            # Prefer the best available video as mux input
            mux_src = None
